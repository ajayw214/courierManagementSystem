...
public interface ICourierRepository extends JpaRepository<Courier, Long>{
	...
	//get all couriers of a customer
		List<Courier> findByCustomerCouriersId(long customerId);	
	...
	//get all couriers by employee id
		//Done with inherited methods
		List<Courier> findByAllotedToDeliveryBoyId(long empId);
	//get a courier detail by courierId by deliveryBoy
		//Done with inherited methods
	//get all couriers which are in transit state 
		List<Courier> findByStatus(StatusEnum valueOf);
	...
	//get all couriers to be picked up/delivered by branch admin
		List<Courier> findByAllotedToDeliveryBoyIdAndStatus(long empId, StatusEnum valueOf);
